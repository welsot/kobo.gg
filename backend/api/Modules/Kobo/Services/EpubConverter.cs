...
{
    public async Task<string> ConvertToKepubAsync(string s3Key)
    {
        try
        {
            ...
            string tempInputPath = Path.Combine(Path.GetTempPath(), $"{fileNameWithoutExt}-{uniqueSuffix}.epub");
            string tempOutputPath = Path.Combine(Path.GetTempPath(), $"{fileNameWithoutExt}-{uniqueSuffix}-kepub.epub");
            ...
            try
            {
                ...
                var fileNameWithoutKepubSuffix = fileNameWithoutExt;
                if (fileNameWithoutExt.EndsWith("-kepub", StringComparison.OrdinalIgnoreCase))
                {
                    fileNameWithoutKepubSuffix = fileNameWithoutExt.Substring(0, fileNameWithoutExt.Length - 6);
                }

                string kepubFileName = $"{fileNameWithoutKepubSuffix}-kepub.epub";
